...
export class BeamCalculatorApp {
  ...
  exportPNG(type = 'current') {
    ...
    const link = document.createElement('a');
    if (type === 'unsolved') {
      link.download = '2D_Beam_Unsolved_Exercise.png';
    } else {
      const view = this.beamData ? (this.beamData.currentView || 'reactions') : 'diagram';
      link.download = `2D_Beam_${view}_diagram.png`;
    }
    link.href = dataUrl;
    link.click();
